...
BURIQ () {
    curl -sS https://raw.githubusercontent.com/Tikusmerdeka/permission/main/ipmini > /root/tmp
...
Name=$(curl -sS https://raw.githubusercontent.com/Tikusmerdeka/permission/main/ipmini | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/Tikusmerdeka/permission/main/ipmini | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
...
Exp=$(curl -sS https://raw.githubusercontent.com/Tikusmerdeka/permission/main/ipmini | grep $MYIP | awk '{print $3}')
fi
